...
export default function SecurityScreen({ onRecheck, violationType = 'DEVELOPER_OPTIONS' }) {
  ...
  const handleOpenSettings = async () => {
    // If Capacitor BackgroundTracking plugin is available
    if (window.Capacitor && window.Capacitor.Plugins && window.Capacitor.Plugins.BackgroundTracking) {
      try {
        await window.Capacitor.Plugins.BackgroundTracking.openDeveloperSettings();
        return;
      } catch (e) {
        console.warn('Could not open developer settings via plugin', e);
      }
    }
    alert('Please open your Android Settings -> System -> Developer options and switch it OFF.');
  };
